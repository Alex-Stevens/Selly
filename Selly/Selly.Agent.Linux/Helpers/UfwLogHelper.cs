...
namespace Selly.Agent.Linux.Helpers
{
    public class UfwLogHelper
    {
        ...
        public UfwLogHelper()
        {
            ...
            proc.StartInfo.Arguments = $"-u \"user\" /usr/bin/tail -f -n0 /var/log/ufw.log";
        ...
        private void OutputDataReceivedOccured(object sender, DataReceivedEventArgs e)
        {
            ...
            try
            {
                _ = SellyService.ApiClient.SendEvent(packet);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"{name}: ERROR in STDOUT: Message: {ex.Message}\n\n");
                Console.WriteLine($"{name}: ERROR in STDOUT: Data: {e.Data}\n\n");
            }
        }
